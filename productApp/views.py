...
def getAllProduct(request):
    ...
    if("search" in body):
        query.update(
            {"title": {"$regex": "|".join(buildQuerySearch(body['search'].split(" ")))}})
    if("category" in body):
        query.update({"category": {"$regex": "|".join(
    ...
    if("sortedBy" in body):
        ...
        # 3 - по спросу ( от большего к меньшему)
        # match body["sortedBy"]:
        #     case 1:
        ...
        pass
    # print(query)
    return JsonResponse(CursorIntoList(Catalog.find(query, limit=30, skip=skip, sort=[]), ["category", "countBuy"]), safe=False)


@api_view(['POST'])
def getCategory(request):
    # print(CursorIntoList(Category.find({}), ["_id"]))
...
def getShops(request):
    # print(CursorIntoList(Category.find({}), ["_id"]))
    return JsonResponse(CursorIntoList(ShopsName.find({}), ["_id", "img"]), safe=False)
